...
export async function swapTaskSort(
    ...
    try {
        if (!target.colId || !selected.id) {
            console.log("Missing Params");
            return;
        }

        const taskInTargetCols = await prisma.task.findMany({
            where: {
                columnId: target.colId,
            },
        });

        if (!taskInTargetCols.length) {
            ...
            const updatedTasks = tasks
                ...
                .reduce((acc, task, index) => {
                    if (task.id === targetTask.id) {
                        if (selectedTask.sort < targetTask.sort) {
                            acc.push({ ...task, sort: index }); // target stays
                            acc.push({ ...selectedTask, sort: index + 1 }); // selected goes below
                        } else {
                            acc.push({ ...selectedTask, sort: index }); // selected goes above
                            acc.push({ ...task, sort: index + 1 }); // target shifts down
                        }
                    } else {
